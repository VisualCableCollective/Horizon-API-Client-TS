...
export class HorizonAPIClient {
  ...
  static RequestService = new HTTPRequestUtil();
  ...
  static async authenticateUserWithToken(token: string) {
    ...
    const response = await HorizonAPIClient.RequestService.Request(GET_SELF_USER_DATA_ROUTE);
    if (response === null) {
      return false;
    }
  ...
  static async getTeam(id: number | string) {
    ...
    const response = await HorizonAPIClient.RequestService.Request(getTeamRouteCopy);
